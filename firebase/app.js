  ...
  onValue(todoListRef, snapshot => {
    ...
    if(isDataExist){
      ...
      snapshot.forEach(data=>{
        ...
        <a id = ${dataKey + '-status'}><i class="fa-solid fa-pen-to-square">${dataValue.status}</i></a>
        </li>`
        ...
        setTimeout(()=>{
          ...
          const statusBtn = document.getElementById(dataKey + '-status')
          editBtn.addEventListener('click' , editFunction)
          deleteBtn.addEventListener('click' , deleteFunction)
          statusBtn.addEventListener('click' , statusFunction)
        },1000)
      })
    }else if(!isDataExist){
      todoConatiner.innerHTML = null
...
function editFunction(){
  ...
  console.log('Edit Button chal raha he')
...
function deleteFunction(){
  ...
  const todoRef = ref(database, `todos/${auth.currentUser.uid}/${elementId}`)
  remove(todoRef)
  console.log('Delete Button chal raha he')
}

function statusFunction(){
  const elementId = this.id.slice(0, this.id.length - 7)
  const todoRef = ref(database, `todos/${auth.currentUser.uid}/${elementId}`)
  update(todoRef, { status:" completed" })

// this.parentNode.style.color= 'red'

  
  // const todoFirstElemet = this.parentNode.child[0]
  // const todoSecondElement = this.parentNode.child[1]

  // console.log(this.parentNode)
  // console.log(todoFirstElemet)
  // console.log(todoSecondElement)


  console.log('Status Button chal raha he')
}